...
test("ambassador ok state shows referral dashboard", async () => {
  const markup = renderToStaticMarkup(await renderAmbassadorPage("zh-HK", { dashboardState: { kind: "ok", dashboard } }));
...
test("ambassador expired_session shows relogin", async () => {
  const markup = renderToStaticMarkup(await renderAmbassadorPage("zh-HK", { dashboardState: { kind: "expired_session", status: 401 } }));
  assert.match(markup, /登入狀態已過期，請重新登入。/);
});

test("ambassador unavailable shows retry not signed-out CTA", async () => {
  const markup = renderToStaticMarkup(await renderAmbassadorPage("zh-HK", { dashboardState: { kind: "unavailable", status: 500, code: "ambassador_tables_missing", source: "same-site API" } }));
