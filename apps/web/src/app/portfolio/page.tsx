...
export default async function PortfolioPage() {
  ...
  return (
    <main className="stack">
      ...
      <WalletConnectCard linkedWalletAddress={portfolio.linkedWallet?.walletAddress} />

      {portfolio.linkedWallet ? (
        <section className="panel stack">
          <h2 className="section-title">Linked Wallet Record</h2>
      ...
      <section className="grid">
        <article className="panel stack">
          ...
          <p className="muted">Settlement asset: {baseSettlementAsset}. Network: {baseNetworkLabel}. Deposits are credited by verifying an onchain USDC transfer to treasury.</p>
          <p className="muted">Treasury: {baseTreasuryAddress || "Set NEXT_PUBLIC_BASE_TREASURY_ADDRESS"}</p>
          <p className="muted">USDC token: {baseUsdcAddress || "Set NEXT_PUBLIC_BASE_USDC_ADDRESS"}</p>
          <p className="muted">Current implementation verifies an existing transaction hash. Sending funds from wallet is still done in your wallet app.</p>
