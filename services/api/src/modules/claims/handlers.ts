...
export const getClaimableStateForMarket = async (
  ...
  if (market.status !== "resolved" || market.resolutionStatus !== "finalized" || !market.winningOutcomeId) {
    return {
  ...
  return {
    ...
    status:
      existing.status === "claimed" ? "claimed" : claimableAmount > existing.claimedAmount ? "claimable" : "blocked",
  };
};
...
export const getClaims = async (input?: { userId?: string }) => {
  const userId = input?.userId ?? DEMO_USER_ID;
  const db = createDatabaseClient();
  const [claims, resolvedMarkets] = await Promise.all([listClaimsForUser(db, userId), listFinalizedResolvedMarkets(db)]);
